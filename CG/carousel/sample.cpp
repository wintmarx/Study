...
void Display() {
    ...
    constexpr double carousel_radius = 5.;

    glColor3f(1., 0., 0.);
    DrawCircle(carousel_radius, 50);

    // draw the box object by calling up its display list:
    glColor3f(1., 1., 0.);
    constexpr double pi = glm::pi<double>();
    const double rocking_period = 2. * pi * 2. * Time;
    constexpr double rocking_pitch_ampl = 25.;
    constexpr double rocking_vert_ampl = 1.;
    for (size_t i = 0; i < horses_count; ++i) {
        const double phase = 2. * pi / horses_count * i;
        glPushMatrix();
        glRotated(360. * Time, 0., 1., 0.);
        glTranslated(carousel_radius * cos(phase), rocking_vert_ampl * sin(rocking_period + phase), carousel_radius * sin(phase));
        glRotated(90. - phase * 180. / pi, 0., 1., 0.);
        glRotated(rocking_pitch_ampl * cos(rocking_period + phase), 0., 0., 1.);
...
void UpdateCurrentProjection(int proj) {
    if (isInsideView && proj == ORTHO) {
        return;
    }
    NowProjection = proj;
}
...
void UpdateHorsesCount(size_t count) {
    horses_count = count;
