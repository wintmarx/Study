...
// clang-format off
Planet Planets[] = {
    {"Venus", "../assets/venus.bmp", 0.95f, 0, 'v', 0},     
    {"Earth", "../assets/earth.bmp", 1.00f, 0, 'e', 0},
    {"Mars", "../assets/mars.bmp", 0.53f, 0, 'm', 0},       
    {"Jupiter", "../assets/jupiter.bmp", 11.21f, 0, 'j', 0},
    {"Saturn", "../assets/saturn.bmp", 9.45f, 0, 's', 0},   
    {"Uranus", "../assets/uranus.bmp", 4.01f, 0, 'u', 0},
    {"Neptune", "../assets/neptune.bmp", 3.88f, 0, 'n', 0},
};
// clang-format on

const int NUMPLANETS = sizeof(Planets) / sizeof(struct Planet);

enum TextureMode { TM_NO_TEXTURE, TM_REPLACE, TM_MODULATE };
...
void Animate() {
    ...
    Time = (float)ms / MS_PER_CYCLE;  // makes the value of Time between 0.
...
void Display() {
    ...
    double carousel_radius = 3. * Planets[NowPlanet].scale;
    double carousel_freq = 2.;
    constexpr double carousel_y = 0.;
    ...
    glLightModelfv(GL_LIGHT_MODEL_AMBIENT, MulArray3(.5f, (float*)WHITE));

    glEnable(GL_LIGHT0);

    SetPointLight(GL_LIGHT0, carousel_radius * sin(Time * 2 * pi * carousel_freq), carousel_y,
                  carousel_radius * cos(Time * 2 * pi * carousel_freq), light_color.r, light_color.g, light_color.b);

    glEnable(GL_TEXTURE_2D);
    if (NowTextureMode == TM_MODULATE) {
        glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
    } else if (NowTextureMode == TM_REPLACE) {
...
void InitGraphics() {
    ...
    for (auto& planet : Planets) {
        ...
        if (texture == NULL)
            ...
            fprintf(stdout, "Opened '%s': width = %d ; height = %d\n", planet.file, width, height);
...
void InitLists() {
    ...
    for (auto& planet : Planets) {
        planet.displayList = glGenLists(1);
        glNewList(planet.displayList, GL_COMPILE);
        SetMaterial(.8, .8, .8, 15);
        glBindTexture(GL_TEXTURE_2D, planet.texObject);  // MarsTex must have already been created when this is called
        glPushMatrix();
        glScalef(planet.scale, planet.scale, planet.scale);  // scale of mars sphere, from the table
...
void UpdateCurrentPlanet(int planet) {
    NowPlanet = planet;
...
void IterateTextureMode() {
    NowTextureMode = (NowTextureMode + 1) % 3;
...
void Keyboard(unsigned char c, int x, int y) {
    ...
    bool isPlanetHandeledKey = false;
    for (int i = 0; i < NUMPLANETS; i++) {
        if (c == Planets[i].key) {
            UpdateCurrentPlanet(i);
            isPlanetHandeledKey = true;
            break;
        }
    }

    if (!isPlanetHandeledKey) {
        switch (c) {
            case 'q':
            case 'Q':
            case ESCAPE:
